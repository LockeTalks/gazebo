...
function populateFeats() {
	document.getElementById("featSelectionZone").innerHTML = ""; //Start by making sure the selection zone is empty
	var charClass = document.getElementById("classSelect").value; //Determine current character class
	var featSlots = document.getElementById("featsAvailable").value; //Determine number of feats available
	var featNamesArray;
	for (i=0; i<featSlots; i++) {
		var featSelectionZone = document.getElementById("featSelectionZone");
		//Create arrays with the names of all feats available for the character's class
		//Deleted to make find easier
	...
	if (charRace == 'human') {
		...
		switch(feat1) {
			...
			case 	"Iron Will" : 
				...
				break;
			case 	"Leadership" : 
				document.getElementById("featDisplayZone").innherHTML = 'SEEEEEEEE';
				break;
